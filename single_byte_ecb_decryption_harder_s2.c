...
static int oracle(unsigned char *in, size_t in_len, unsigned char *out) {
  ...
  // prepend randomly initialized 'prefix_len' bytes
  // and append 'secret' to end of input
...
static ssize_t detect_block_size() {
  size_t first_increase = 0, second_increase = 0;
  size_t prev_out_size, curr_out_size;
  ...
  res = first_increase == 0 || second_increase == 0
            ? -1
            : second_increase - first_increase;
error:
  free(in);
  free(out);
  return res;
...
static ssize_t detect_total_len(size_t block_size) {
  ssize_t res = -1;
  unsigned char *out =
      malloc(MAX_SECRET_SIZE + 2 * block_size + MAX_PREFIX_SIZE);
  ...
  for (size_t i = 1; i <= block_size; i++) {
    if (oracle(in, i, out) != enc_size) {
      res = (enc_size / block_size - 1) * block_size + (block_size - i);
      break;
    }
  }
early_exit:
...
static ssize_t detect_prefix_len(ssize_t block_size) {
  ssize_t res = -1;
  size_t out_size = MAX_SECRET_SIZE + 2 * block_size + MAX_PREFIX_SIZE;
  unsigned char *out1 = malloc(out_size), *out2 = malloc(out_size);
  unsigned char *one = calloc(block_size, 1), *two = calloc(block_size, 1);
  ...
  for (size_t i = upper_limit; i >= lower_limit; i--) {
    ...
    enc_size = oracle(&one[block_size - pad_size - 1], pad_size + 1, out1);
    oracle(&two[block_size - pad_size - 1], pad_size + 1, out2);
    if (memcmp(out1, out2, i) == 0)
      res = i;
    break;
  }
...
static int decrypt_next_char(size_t block_size, size_t prefix_len,
  ...
  int res = -1;
  unsigned char *in = calloc(block_size, block_size);
  unsigned char *out = malloc(total_size + block_size + MAX_SECRET_SIZE);
  unsigned char *c_block = NULL;
  if (oracle(in, padding_needed, out) < 0)
    goto early_exit;
  // save last relevant block of output for comparison
  unsigned char *last_relevant_block = out + (blocks_needed - 1) * block_size;
  c_block = malloc(block_size);
  memcpy(c_block, last_relevant_block, block_size);
  // copy 'known_chars' into input for checking
  ...
  for (uint i = 0; i < 256; i++) {
    ...
    if (oracle(in, total_size - prefix_len, out) < 0)
      goto early_exit;
    if (memcmp(last_relevant_block, c_block, block_size) == 0)
      res = i;
    break;
  }
early_exit:
...
int main() {
  ...
  printf("block_size=%zd, total_len=%zd, prefix_len=%zd, "
         ...
         block_size, total_len, prefix_len, guessed_prefix_len, secret_len);
  // decrypt message byte-by-byte using oracle
  unsigned char *secret = calloc(MAX_SECRET_SIZE, 1);
  for (size_t i = 0; i < (size_t)secret_len; i++) {
    int next_char =
        decrypt_next_char(block_size, guessed_prefix_len, secret, i);
    if (next_char < 0) {
      printf("cannot decrypt next char D:\n");
      exit(1);
    }
