...
static ssize_t detect_block_size() {
  size_t first_increase = 0, second_increase = 0, prev_out_size, curr_out_size;
...
static int decrypt_next_char(size_t block_size, unsigned char *known_chars,
                             size_t known_len) {
  ...
  unsigned char *out =
      malloc((blocks_needed + 1) * block_size + MAX_SECRET_SIZE);
  if (oracle(in, padding_needed, out) < 0) {
    free(in);
    free(out);
    return -1;
  }
  // save last relevant block of output for later comparison
...
int main() {
  ...
  if (s_block_size < 0) {
    fprintf(stderr, "unable to detect block_size\n");
    exit(1);
  }
  size_t block_size = (size_t)s_block_size;
  int is_ecb = detect_ecb(block_size);
  printf("detected block size: %zu, ecb: %d\n", block_size, is_ecb);
